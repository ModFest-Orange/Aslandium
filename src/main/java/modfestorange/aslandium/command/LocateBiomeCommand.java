 ...
 * Stolen from https://github.com/TerraformersMC/Terraform
 */
public class LocateBiomeCommand {
    private static int timeout = 120_000;

    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        LiteralArgumentBuilder<ServerCommandSource> builder = CommandManager.literal("locatebiome").requires(source ->
    ...
    private static BlockPos spiralOutwardsLookingForBiome(ServerCommandSource source, World world, Biome biomeToFind, double startX, double startZ) throws CommandSyntaxException {
        ...
        for (int n = 0; dist < Integer.MAX_VALUE; ++n) {
            ...
            if (source.getEntity() instanceof PlayerEntity && !(source.getMinecraftServer() instanceof DedicatedServer))
                source.getPlayer().sendChatMessage(new TranslatableText("commands.terraform.locatebiome.scanning", dots), MessageType.GAME_INFO);
            if (i == 9216) {
                previous++;
                i = 0;
            }
            ...
            if (world.getBiome(pos).equals(biomeToFind)) {
                pos.close();
                if (source.getEntity() instanceof PlayerEntity && !(source.getMinecraftServer() instanceof DedicatedServer))
                    source.getPlayer().sendChatMessage(new TranslatableText("commands.terraform.locatebiome.found", new TranslatableText(biomeToFind.getTranslationKey()), (System.currentTimeMillis() - start) / 1000), MessageType.GAME_INFO);
